...
int main( int argc, char** argv )
{
  if ( argc != 3 )
  {
    std::cerr << "Usage: extract_info_dump <replay file> <dump file>" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  replay.getInfo().exportDump( argv[2] );
  
  std::cout << "Replay info successfully extracted to \"" << argv[2] << "\"." << std::endl;
